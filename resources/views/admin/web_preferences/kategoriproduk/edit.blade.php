...
@section('content')
    ...
    <div x-data="{ image1Preview: null, image2Preview: null }"
        ...
        <form action="/admin/web-preferences/kategori/{{ $kategori->id }}" method="POST" class="space-y-6" id="category-form">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <div id="editor-container">
                    {!! old('deskripsi', $kategori->deskripsi) !!} {{-- Mengisi editor dengan data lama jika ada --}}
                </div>
                @error('deskripsi')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
