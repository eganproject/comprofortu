...
@section('content')
    ...
    <h2 class="text-2xl font-bold text-slate-800 mb-6">Ubah Post</h2>
    ...
    <div x-data="{ image1Preview: '{{ $blog->image ? asset('storage/' . $blog->image) : null }}' }"
        ...
        <form action="/admin/web-preferences/blog/{{ $blog->id }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-slate-500">Kosongkan jika tidak ingin mengubah gambar. PNG, JPG, WEBP, SVG (MAX. 2MB). Rekomendasi rasio 16:9.</p>
                @error('image')
